...
- The signed arm64 archive and App Store export passed. App Store Connect
  accepted build 61 and reports it processing. The same non-blocking missing
  dSYM warnings remain limited to prebuilt Expo/React dependencies.
...
- Removing that feedback exposed that direct `NSClipView.scroll(to:)` calls had
  ...
  clip view now constrains both `scroll(to:)` and `setBoundsOrigin`
  synchronously; clamping is no longer an accidental side effect of the removed
  feedback loop.
...
- Extended the dock regression to prove the dragged height reaches the mounted
  ...
  Electron tests pass.
- Added a short, debounced reconciliation through the structured terminal
