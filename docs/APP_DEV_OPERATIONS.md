...
| Updates | Validate installed-version → candidate update, failed/offline behavior, retained settings and active-work safety; do not infer install success from download success. Include an existing server with legacy installation-root permissions (`0755` and `0750`), not only a fresh fixture created under a restrictive umask. Exercise failure before service takeover, preserved incumbent health, cleanup of the exact update fence, and retry after a previously failed installer has removed its candidate stage. |
