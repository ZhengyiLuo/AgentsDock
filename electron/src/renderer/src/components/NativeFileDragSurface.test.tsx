...
describe('NativeFileDragSurface', () => {
  it('starts an on-demand native file drag on the first gesture', async () => {
    ...
    const { getByText } = render(<NativeFileDragSurface sessionId="chat-1" file={file}><span>Result</span></NativeFileDragSurface>)
    const surface = getByText('Result').closest('article')!

    fireEvent.dragStart(surface)

    await waitFor(() => expect(beginDrag).toHaveBeenCalledWith('chat-1', file))
  ...
  it('shares one in-flight native drag request', async () => {
    ...
    const { getByText } = render(<NativeFileDragSurface sessionId="chat-1" file={file}><span>Slow result</span></NativeFileDragSurface>)
  ...
  it('does not start a native drag from an action button', async () => {
    ...
    const { getByText } = render(<NativeFileDragSurface sessionId="chat-1" file={file}><button data-native-drag-ignore>Download</button></NativeFileDragSurface>)
