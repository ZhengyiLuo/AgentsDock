...
describe('Codex custom endpoint settings', () => {
  ...
  it('tests exact endpoint/model/key without saving, then saves only after a successful current test', async () => {
    ...
    expect(screen.getByText(/Native Codex · Responses API required/)).toHaveAttribute('title', 'Used for new Codex chats. Existing chats keep their provider binding; switch back to continue them.')
    fillProvider(fields)
    expect(api.testProvider).not.toHaveBeenCalled()
    ...
    await screen.findByText(/Endpoint saved for new Codex chats/)
  ...
  it('reads configured endpoints on opening and never exposes default sign-in or reuses a saved key', async () => {
  ...
  it('resets the custom endpoint explicitly without signing in or sending a saved key', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('button', { name: 'Use API key' })).toBeEnabled())
    fireEvent.click(screen.getByRole('button', { name: 'Use API key' }))
    await screen.findByRole('button', { name: 'Use Codex default' })
    fireEvent.click(screen.getByRole('button', { name: 'Use Codex default' }))
    await screen.findByText('Original Codex configuration restored. Your login is unchanged.')
  ...
  it('recovers unreadable provider settings with explicit reset and permits retry after busy rejection', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('button', { name: 'Use API key' })).toBeEnabled())
    fireEvent.click(screen.getByRole('button', { name: 'Use API key' }))
    const reset = await screen.findByRole('button', { name: 'Use Codex default' })
  ...
  it.each(['CODEX_PROVIDER_ADMIN', 'CODEX_PROVIDER_UPDATE', 'CODEX_PROVIDER_CONNECTION'])('does not offer metadata recovery for %s', async error => {
    ...
    expect(screen.queryByRole('button', { name: 'Use Codex default' })).not.toBeInTheDocument()
