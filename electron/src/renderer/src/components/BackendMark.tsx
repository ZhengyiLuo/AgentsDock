import { Bot, MousePointer2, TerminalSquare } from 'lucide-react'
  ...
  return (
    <span className={`backend-mark backend-${backend}`} style={{ width: size, height: size }} aria-hidden="true">
      ...
      <span className="backend-fallback">
        {backend === 'codex' ? <TerminalSquare size={size - 3} /> : backend === 'cursor' ? <MousePointer2 size={size - 3} /> : <Bot size={size - 3} />}
      </span>
    </span>
  )
}
