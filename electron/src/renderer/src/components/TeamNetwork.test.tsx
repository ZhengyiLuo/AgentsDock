...
describe('Team Network', () => {
  ...
  it('changes the saved host address from the offline recovery card and restores Team Network', async () => {
    ...
    const recoveredControl = deferred<SecurePeerControlStatus>()
    teamHub.status.mockResolvedValue(peerStatus)
    teamHub.securePeerStatus.mockReturnValue(recoveredControl.promise)
    fireEvent.click(within(dialog).getByRole('button', { name: 'Save' }))
    expect(await screen.findByText('Initial update')).toBeVisible()
    // The recovered header loads its control status independently of the feed.
    expect(screen.queryByTitle('Current host address: 100.64.0.9:7851')).not.toBeInTheDocument()
    await act(async () => recoveredControl.resolve(nextControl))
    expect(await screen.findByTitle('Current host address: 100.64.0.9:7851')).toBeVisible()
