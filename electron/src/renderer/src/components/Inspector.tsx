...
function MediaInspector({ sessionId, files, total, loading, loadMore, onPreview }: { sessionId: string; files: AgentFile[]; total: number; loading: boolean; loadMore: () => void; onPreview: (file: AgentFile) => void }) {
  ...
  return <div className="media-inspector"><div className="inspector-media-grid">{media.map(file => <article key={file.id} draggable onDragStart={event => { event.preventDefault(); window.agentsDock.files.beginDrag(file) }}><button className="inspector-thumb" onClick={() => onPreview(file)}>{file.content_type?.startsWith('image/') ? <img src={window.agentsDock.files.mediaURL(file.id)} loading="lazy" /> : <LazyVideoThumbnail source={window.agentsDock.files.mediaURL(file.id)} />}{file.content_type?.startsWith('video/') && <Play className="thumb-play" size={14} fill="currentColor" />}</button><strong title={file.title || file.filename}>{file.title || file.filename}</strong><small>{formatBytes(file.size)}</small><div><button title="Find in chat" onClick={() => window.dispatchEvent(new CustomEvent('agentsdock:find-file', { detail: file.id }))}><RefreshCw size={11} /></button><button title="Pin" onClick={() => void pin(file)}><Pin size={11} /></button><button title="Download" onClick={() => void window.agentsDock.files.save(file)}><Download size={11} /></button><button title="Reveal in Finder" onClick={() => void window.agentsDock.files.reveal(file)}><FolderOpen size={11} /></button><button title="Open" onClick={() => void window.agentsDock.files.open(file)}><ExternalLink size={11} /></button></div></article>)}</div>{documents.length > 0 && <div className="document-list">{documents.map(file => <article key={file.id} draggable onDragStart={event => { event.preventDefault(); window.agentsDock.files.beginDrag(file) }}><File size={14} /><span><strong title={file.filename}>{file.filename}</strong><small>{formatBytes(file.size)}</small></span><button title="Find in chat" onClick={() => window.dispatchEvent(new CustomEvent('agentsdock:find-file', { detail: file.id }))}><RefreshCw size={12} /></button><button title="Download" onClick={() => void window.agentsDock.files.save(file)}><Download size={12} /></button><button title="Reveal in Finder" onClick={() => void window.agentsDock.files.reveal(file)}><FolderOpen size={12} /></button></article>)}</div>}{files.length < total && <button className="load-more" disabled={loading} onClick={loadMore}>{loading && <LoaderCircle className="spin" size={13} />} Load 60 more</button>}</div>
}
