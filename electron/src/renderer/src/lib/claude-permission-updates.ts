...
export type ClaudePermissionPatch = Pick<Session, 'claude_permission_mode'>

const pendingBySession = new Map<string, Promise<void>>()

export function queueClaudePermissionUpdate(sessionId: string, patch: ClaudePermissionPatch): Promise<void> {
  ...
  const operation = previous.catch(() => undefined).then(async () => {
    ...
    if (!updated || Object.entries(patch).some(([key, value]) => updated[key as keyof Session] !== value)) {
      throw new Error(useAppStore.getState().error || 'Could not update Claude permissions.')
    }
  })
...
export function awaitClaudePermissionUpdates(sessionId: string): Promise<void> {
