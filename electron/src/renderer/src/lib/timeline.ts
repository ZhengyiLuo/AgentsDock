  ...
  private rememberCrossChatDeliveryAliases(events: Event[]): boolean {
    ...
    for (const event of this.importedCrossChatInputs.values()) {
      ...
      if (hasProviderUserProvenance(event)) {
        ...
        // `user.text`. Recover only when the provider thread + turn identity
        // uniquely proves which native cross-chat run owned that exact input.
        // A pasted lookalike, missing identity, or ambiguous owner stays visible.
        if (providerTurnMatches.length !== 1) continue
        const runId = providerTurnMatches[0][0]
