...
class TeamMessagesServiceTests(unittest.TestCase):
    ...
    def test_skill_lifecycle_create_update_cas_pin_archive_restore(self) -> None:
        first = self.skill_post(
            ...
            "Deploy ATLAS",
            "# Steps\n1. build\n2. ship",
            summary="How we deploy ATLAS",
            tags=["Atlas", "deploy", "atlas"],
        )["message"]
        self.assertEqual(first["kind"], "skill")
        self.assertEqual(first["skill"]["slug"], "deploy-atlas")
        ...
        skill = listed[0]
        self.assertEqual(skill["slug"], "deploy-atlas")
        self.assertEqual(skill["tags"], ["atlas", "deploy"])
        ...
        by_slug = self.get(self.guest, f"{self.base}/skills?slug=deploy-atlas")["skills"]
        self.assertEqual([item["id"] for item in by_slug], [skill_id])

        # Updating requires the current version.
        conflict = self.skill_post(
            self.member, "deploy-atlas", "Deploy ATLAS", "# v2", expected=409
        )
        self.assertEqual(conflict["error"]["code"], "skill_version_conflict")
        stale = self.skill_post(
            self.member, "deploy-atlas", "Deploy ATLAS", "# v2", expected=409, expected_version=5
        )
        ...
        self.assertEqual(detail["title"], "Deploy ATLAS v2")
        ...
        other = self.skill_post(self.owner, "run-robot", "Run ROBOT", "# robot")["message"]
        pinned = self.post(
            self.member,
        ...
        self.assertEqual(order, ["deploy-atlas", "run-robot"])
        ...
        self.assertEqual(visible, ["run-robot"])
        everything = [
            item["slug"]
            for item in self.get(self.guest, f"{self.base}/skills?include_archived=1")["skills"]
        ]
        self.assertEqual(sorted(everything), ["deploy-atlas", "run-robot"])
        blocked = self.skill_post(
            self.owner, "deploy-atlas", "Deploy ATLAS v3", "# v3", expected=409, expected_version=2
        )
        ...
        third = self.skill_post(
            self.owner, "deploy-atlas", "Deploy ATLAS v3", "# v3", expected_version=2
        )["message"]
