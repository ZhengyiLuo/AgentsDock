...
import { loadTeamMentionCandidates, teamMessagesAvailable, type TeamMentionCandidate } from '../lib/team-references'
...
export function TeamTargetPicker({ visible, width, query, sourceSessionId, referenceLimitReached, onQueryChange, onSelect, onClose, onDidDismiss }: {
  ...
  useEffect(() => {
    ...
    return () => { live = false; selectionCurrent.current = null }
  }, [visible, profileId, generation, sourceSessionId, connected, connecting, switching, available, routeKey, connection, validationRevision, retry])

  const needle = query.trim().toLocaleLowerCase()
  const targets = candidates.filter(candidate => !needle || [candidate.label, candidate.teamName, candidate.target.target_id].some(value => value.toLocaleLowerCase().includes(needle)))
