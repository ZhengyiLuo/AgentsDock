...
export function TeamTargetPicker({ visible, width, query, sourceSessionId, referenceLimitReached, onQueryChange, onSelect, onClose, onDidDismiss }: {
  ...
  return <AppOverlay visible={visible} onShow={() => search.current?.focus()} onClose={onClose} onDidDismiss={onDidDismiss}>
    {visible ? <SafeAreaView edges={['top', 'bottom', 'left', 'right']} style={[styles.safe, { backgroundColor: colors.background }]} onAccessibilityEscape={onClose}>
      <KeyboardAvoidingView testID="team-target-keyboard-safe" automaticOffset behavior="padding" style={styles.keyboardSafe}>
      <View style={[styles.panel, width >= 720 && styles.panelTablet]}>
...
const styles = StyleSheet.create({
  panel: { flex: 1, minHeight: 0, width: '100%', alignSelf: 'center' }, panelTablet: { maxWidth: 760 },
