...
export function Inspector({ sessionId, onDigest, onJob, onTerminal, onProcesses }: { sessionId: string; onDigest: () => void; onJob: () => void; onTerminal: () => void; onProcesses: () => void }) {
  ...
  const jobs = useAppStore(state => state.jobs.filter(value => value.session_id === sessionId))
  const pins = useAppStore(state => state.pins.filter(value => value.sessionId === sessionId))
...
function Command({ icon: Icon, label, onPress, destructive }: { icon: typeof Pin; label: string; onPress: () => void; destructive?: boolean }) { const colors = usePalette(); return <Pressable onPress={onPress} style={({ pressed }) => [styles.command, { backgroundColor: colors.raised, opacity: pressed ? 0.65 : 1 }]}><Icon size={15} color={destructive ? colors.red : colors.muted} /><Text style={{ color: destructive ? colors.red : colors.text, fontSize: 11, fontWeight: '700' }}>{label}</Text></Pressable> }
