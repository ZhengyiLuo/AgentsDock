...
export function isChatMailboxEvent(event: Event): boolean {
  return event.conversation_mode === 'async_route_v1' && event.delivery_mode === 'mailbox'
    && /^chat_conversation_message_(registered|received|mailbox_migrated|read|cancelled|deleted)$/u.test(event.type)
}
...
export function parseChatInboxPage(value: unknown, sessionId: string, limit: number): ChatInboxPage {
  if (!Number.isInteger(limit) || limit < 1 || limit > 25 || !record(value)
    || value.session_id !== sessionId || !Array.isArray(value.messages) || value.messages.length > limit
    || !Array.isArray(value.senders) || typeof value.has_more !== 'boolean'
    || !(value.next_cursor === null || typeof value.next_cursor === 'string' && /^\d+$/u.test(value.next_cursor))
  ...
  for (const message of value.messages) {
    if (!record(message) || !text(message.message_id) || ids.has(message.message_id)
      ...
      || typeof message.state !== 'string' || !['unread', 'read', 'cancelled', 'deleted'].includes(message.state)
      || typeof message.body !== 'string' || !count(message.body_chars) || !count(message.message_revision)
      || typeof message.body_sha256 !== 'string' || !/^[a-f0-9]{64}$/iu.test(message.body_sha256)
...
export function inboxMessageMatchesEvent(message: ChatInboxMessage, event: Event): boolean {
  return isChatMailboxEvent(event) && message.message_id === chatInboxMessageId(event)
    && message.source_session_id === event.source_session_id && message.target_session_id === event.target_session_id
    && message.conversation_id === event.conversation_id && message.message_revision === (event.message_revision ?? 0)
}
