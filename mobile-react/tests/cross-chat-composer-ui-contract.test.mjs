...
test('touch-first composer offers a searchable adaptive target-chat picker', () => {
  ...
  assert.match(picker, /return <AppOverlay/)
  assert.doesNotMatch(picker, /<Modal|allowSwipeDismissal/)
  assert.match(composer, /onShow=\{\(\) => searchInputRef\.current\?\.focus\(\)\}/)
  assert.match(composer, /onDidDismiss=\{onDidDismiss\}/)
  ...
  assert.doesNotMatch(composer, /if \(Platform\.OS !== 'ios'\) requestAnimationFrame\(finishTargetPickerDismissal\)/)
