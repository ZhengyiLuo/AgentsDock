...
func checkMacTimelineScrollPerformanceGuards() throws {
    ...
    try assert(appKitTimeline.contains("guard !event.hasPreciseScrollingDeltas") && appKitTimeline.contains("scrollWheelEventIsContinuous") && appKitTimeline.contains("setPixelDelta(event.scrollingDeltaY"), "Every non-precise wheel event must change units to continuous pixels before NSTableView interprets it as variable-height rows")
