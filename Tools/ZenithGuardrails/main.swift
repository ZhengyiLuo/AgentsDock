...
func checkComposerUsesPresenceGate() throws {
    ...
    try assert(source.contains("textView.string = \"\"\n            parent.onDraftChange(\"\")"), "Composer submit must clear the native text view immediately after accepting a non-empty submit")
...
func checkTimelineRevealWaitsForLatestSnapshot() throws {
    ...
    try assert(mobileStore.contains("beginLargeTimelineBatchMaskIfCold"), "iOS store must mask cold large session snapshots before publishing rows")
