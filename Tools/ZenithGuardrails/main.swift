...
func checkRuntimeDefaultLabels() throws {
    ...
    try assert(server.contains("discovered_codex_default_model") && server.contains("slug == CODEX_DEFAULT_MODEL"), "Server Codex catalog must prefer its explicit launch default when present")
    ...
    try assert(server.contains("\"max\": \"xhigh\""), "Server must translate legacy Claude Max effort to Codex XHigh")
    ...
    try assert(server.contains("normalized_effort = normalize_runtime_effort(") && server.contains("sess.get(\"effort\") or CODEX_DEFAULT_EFFORT"), "Codex launch must normalize persisted effort and explicitly apply its advertised default")
...
func checkClaudeResumeFailureDoesNotPoisonSession() throws {
    ...
    try assert(server.contains("if codex_error_emitted:\n            pass\n        elif codex_error:"), "Structured Codex failures must suppress duplicate stderr error cards")
    ...
    try assert(server.contains("sess.get(\"effort\") or CODEX_DEFAULT_EFFORT"), "Fresh Codex turns must pass the advertised default effort explicitly")
