...
func checkMacTimelineScrollPerformanceGuards() throws {
    ...
    try assert(timeline.contains(".defaultScrollAnchor(.bottom)") && timeline.contains(".id(documentIdentity)"), "Lazy chat opening must use a session-scoped native bottom anchor")
