...
func checkEndpointCacheKeysAreServerScoped() throws {
    ...
    let first = ZEndpointCache.key(serverURL: "http://100.64.0.4:7850", sessionID: sessionID, default: fallback)
    let second = ZEndpointCache.key(serverURL: "100.64.0.1:7850/api/health", sessionID: sessionID, default: fallback)
...
func checkServerURLNormalization() throws {
    ...
    try assert(
        ZenithServerURL.normalized("100.64.0.1:7850/api/health", default: fallback) == "http://100.64.0.1:7850",
        "health endpoint paste should normalize to server root"
    )
    try assert(
        ZenithServerURL.normalized("http://100.64.0.1:7850/", default: fallback) == "http://100.64.0.1:7850",
...
func checkVideoMetadataIsNotHiddenByMixedFilePaging() throws {
    ...
    try assert(macStore.contains("URLQueryItem(name: \"content_prefix\", value: \"image/\")"), "Mac store must fetch images independently of mixed file paging")
    try assert(macStore.contains("URLQueryItem(name: \"limit\", value: \"\\(sessionFilesPageLimit)\")"), "Mac visual metadata fetch must be paged instead of pulling every media item while switching chats")
    try assert(inspector.contains("store.sessionVideos"), "Mac files inspector must render the independent video list")
    try assert(inspector.contains("store.sessionImages"), "Mac files inspector must render the independent image list")
...
func checkMacTimelineScrollPerformanceGuards() throws {
    ...
    try assert(macStore.contains("@Published private(set) var sessionImages: [ZFile] = []"), "Mac store must cache selected-session images instead of deriving them during render")
    ...
    try assert(!macStore.contains("var sessionImages: [ZFile] {\n        mergedFiles"), "Mac sessionImages must not be a merge/sort computed getter")
    try assert(inspector.contains("for file in store.sessionVideos + store.sessionImages + images"), "Files inspector must use cached visual ordering before file-page image previews")
    ...
    try assert(!appKitTimeline.contains("noteUserBoundsChange()") && appKitTimeline.contains("noteWheelInput(_ sample:"), "Programmatic bounds changes must not masquerade as a new user scroll gesture")
    ...
    try assert(appKitTimeline.contains("AppKitTimelineWheelRouting.delivery") && appKitTimeline.contains("super.scrollWheel(with: delivery.event)"), "Timeline wheel input must retain exactly one AppKit dispatch")
    try assert(appKitTimeline.contains("guard !event.hasPreciseScrollingDeltas") && appKitTimeline.contains("scrollWheelEventIsContinuous") && appKitTimeline.contains("preferredPixelDelta(rawPointDelta: rawPointDeltaY"), "Every non-precise wheel event must retain the driver's accelerated point delta when changing to continuous pixels")
    try assert(appKitTimeline.contains("scrollWheelEventPointDeltaAxis1") && appKitTimeline.contains("abs(rawPointDelta) > 0.01 ? rawPointDelta : fallback"), "Fast mouse-wheel input must use native point travel instead of capped line units")
    try assert(!appKitTimeline.contains("legacySmoothThreshold") && !appKitTimeline.contains("legacyPixelStreamDeadline") && !appKitTimeline.contains("shouldStartLegacyPixelStream"), "A gesture must never mix legacy row units with continuous pixel units")
    try assert(!appKitTimeline.contains("min(48") && !appKitTimeline.contains("tunedDelta") && !appKitTimeline.contains("normalizedLegacyDelta") && !appKitTimeline.contains("scaleIntegerField"), "Timeline pixel compatibility must not reintroduce a speed cap or scaling curve")
    try assert(appKitTimeline.contains("native-wheel-passthrough") && appKitTimeline.contains("native-legacy-wheel-behavior") && appKitTimeline.contains("legacy-smooth-pixel-compatibility") && appKitTimeline.contains("mixed-legacy-stream-stability"), "Precise, tiny legacy, smooth legacy, and mixed-stream wheel paths must all retain harness coverage")
    try assert(appKitTimeline.contains("canRearmAtTop: deferredItems == nil && !currentDocumentIsScrollable()"), "Older-history paging must not chain another page while the first prepend is deferred")
    try assert(appKitTimeline.contains("AppKitTimelineClampingClipView") && appKitTimeline.contains("override func scroll(to newOrigin:") && appKitTimeline.contains("override func setBoundsOrigin") && appKitTimeline.contains("native-overscroll-clamping"), "Native scrolling must synchronously clamp every clip-origin entry point without changing wheel deltas")
    ...
    try assert(appKitTimeline.contains("measureVisibleHeightsNow(in: tableView)") && appKitTimeline.contains("measureAndReportHeightNow()"), "Explicit chat positioning must settle visible row heights before writing the final scroll origin")
    ...
    try assert(testBuild.contains("SWIFT_ACTIVE_COMPILATION_CONDITIONS=$(inherited) AGENTSDOCK_APPKIT_TIMELINE") && !testBuild.contains("SWIFT_ACTIVE_COMPILATION_CONDITIONS=$(inherited) AGENTSDOCK_LAZY_TIMELINE"), "AgentsDock-test must exercise the native recycled timeline without compiling LazyVStack")
    ...
    try assert(timeline.contains("give LazyVStack a\n        // stable rendered tail") && timeline.contains("let renderedVisibleRowLimit = visibleRowLimit"), "The lazy timeline must keep its initial variable-height document bounded")
    try assert(timeline.contains("let rows = Array(allRows.suffix(renderedVisibleRowLimit))"), "LazyVStack must render the stable visible tail instead of the full cached transcript")
    ...
    try assert(!timeline.contains(".frame(maxWidth: .infinity, alignment: .leading)\n        case .bottom:"), "Lazy timeline rows must not renegotiate an infinite width beneath a finite-width lazy stack")
    ...
    try assert(core.contains("usesIsolatedTestCredential") && !core.contains("#if AGENTSDOCK_APPKIT_TIMELINE") && testBuild.contains("agentAccessToken"), "Test credential isolation must follow the test bundle identity rather than a timeline implementation flag")
    ...
    try assert(appKitTimeline.contains("private struct AppKitTimelinePagingController"), "Native history paging must have one explicit gesture-owned state machine")
    try assert(appKitTimeline.contains("native-top-paging-coordinator"), "Native history paging must test the real coordinator callback on short content")
