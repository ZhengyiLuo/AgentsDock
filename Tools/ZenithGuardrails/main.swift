...
func checkTimelineRevealWaitsForLatestSnapshot() throws {
    ...
    try assert(timeline.contains("pendingOpenBottomSessionID = nil\n        historyLoadSuppressedUntil"), "Mac timeline must consume the open-position request once rows are loaded")
...
func checkTimelineHistoryPaging() throws {
    ...
    try assert(timeline.contains("private func renderedRows(visibleLimit:"), "Older-page navigation must target the same projected rows the UI actually renders")
