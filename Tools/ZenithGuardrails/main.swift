...
func checkMacTimelineScrollPerformanceGuards() throws {
    ...
    try assert(timeline.contains(".defaultScrollAnchor(.bottom, for: .initialOffset)") && timeline.contains(".id(documentIdentity)"), "Lazy chat opening must use a session-scoped initial bottom anchor")
    try assert(!timeline.contains(".defaultScrollAnchor(.bottom)\n"), "Lazy live updates must not reuse the initial bottom anchor for content-size changes")
    ...
    try assert(timeline.contains("Consuming the pending open here must not create a scroll action"), "Lazy chat opening must not chase an unmeasured bottom with ScrollViewReader")
