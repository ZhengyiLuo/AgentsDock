...
func checkArchiveSessionBehavior() throws {
    ...
    try assert(!macSidebar.contains(".onDrag"), "Mac sidebar must not make chats or folders draggable")
    try assert(!macSidebar.contains(".onDrop"), "Mac sidebar must not use drag/drop reorder targets")
    try assert(macSidebar.contains("Move chat up"), "Mac sidebar reorder mode must expose explicit chat move controls")
    try assert(macSidebar.contains("Move folder up"), "Mac sidebar reorder mode must expose explicit folder move controls")
...
func checkTimelineRevealWaitsForLatestSnapshot() throws {
    ...
    try assert(timeline.contains("let timelineRowsStructurallySuspended = timelineRowsSuspended || store.isApplyingLargeTimelineBatch"), "Mac timeline must structurally suspend rows for large sync batches")
    try assert(timeline.contains("let shouldMaskTimeline = timelineRowsStructurallySuspended || store.isRefreshingCachedDelta"), "Mac timeline should show the opening mask while a cached chat or large batch refreshes")
