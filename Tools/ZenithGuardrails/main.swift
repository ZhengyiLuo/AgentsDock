...
func checkMacTimelineScrollPerformanceGuards() throws {
    ...
    try assert(appKitTimeline.contains("useLegacyPixelCompatibility") && appKitTimeline.contains("scrollWheelEventIsContinuous") && appKitTimeline.contains("setPixelDelta(event.scrollingDeltaY"), "Mislabelled smooth legacy input must change units to continuous pixels before NSTableView interprets it as variable-height rows")
    try assert(appKitTimeline.contains("legacySmoothThreshold: CGFloat = 1.25") && appKitTimeline.contains("legacyPixelStreamDeadline = now + 0.38"), "Legacy pixel compatibility must include the complete smooth gesture tail")
    ...
    try assert(appKitTimeline.contains("native-wheel-passthrough") && appKitTimeline.contains("native-legacy-wheel-behavior") && appKitTimeline.contains("legacy-smooth-pixel-compatibility"), "Precise, ordinary legacy, and mislabelled smooth wheel paths must all retain harness coverage")
