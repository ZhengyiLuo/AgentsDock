...
func checkTimelineRevealWaitsForLatestSnapshot() throws {
    ...
    try assert(timeline.contains("pendingOpenBottomSessionID = nil\n        suppressHistoryLoading(for: 1.4)"), "Mac timeline must consume the open-position request once rows are loaded")
...
func checkTimelineHistoryPaging() throws {
    ...
    try assert(timeline.contains("suppressHistoryLoading(for: 1.4)"), "Opening a chat must suppress top-edge history loading while bottom scrolling settles")
