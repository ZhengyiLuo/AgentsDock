...
func checkMacTimelineScrollPerformanceGuards() throws {
    ...
    try assert(appKitTimeline.contains("super.scrollWheel(with: event)") && !appKitTimeline.contains("let pointDelta = event.hasPreciseScrollingDeltas"), "Timeline wheel input must preserve native AppKit acceleration and momentum instead of assigning clip origins manually")
