...
func checkMacTimelineScrollPerformanceGuards() throws {
    ...
    try assert(appKitTimeline.contains("routeWheelEventIfNeeded") && appKitTimeline.contains("routeVerticalWheel(event)") && appKitTimeline.contains("return nil"), "Hosted timeline wheel input must be routed through one native scroll owner exactly once")
