...
func checkMacTimelineScrollPerformanceGuards() throws {
    ...
    try assert(!testBuild.contains("SWIFT_ACTIVE_COMPILATION_CONDITIONS=$(inherited) AGENTSDOCK_LAZY_TIMELINE") && !testBuild.contains("SWIFT_ACTIVE_COMPILATION_CONDITIONS=$(inherited) AGENTSDOCK_APPKIT_TIMELINE"), "AgentsDock-test must use the bounded eager timeline; neither experimental virtualizer is stable enough to ship")
