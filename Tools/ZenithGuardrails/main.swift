...
func checkMacTimelineScrollPerformanceGuards() throws {
    ...
    try assert(appKitTimeline.contains("queueCachedHeightCorrectionIfNeeded") && appKitTimeline.contains("maxY >= visibleTop"), "Measured rows above the viewport must remain cached until they become visible")
    ...
    try assert(appKitTimeline.contains("!isLiveScrolling") && appKitTimeline.contains("else if shouldRestoreAnchor, let anchor"), "Live scrolling must suppress passive anchor writes")
